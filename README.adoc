...
Once both virtual servers -storage and applications- are up&running, it's time to deploy customer-facing applications over them. In the following sections, a *functional description* of the applications and its *logical architecture* are presented. 
