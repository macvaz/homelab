...
Proxmox creates a default ** https://pve.proxmox.com/wiki/Network_Configuration[virtual bridged network] (vmbr0)** linked with the default ** ethernet physical NIC eno1** in such a manner each NIC of a VM belongs to the physical home network (192.168.1.0/24) managed by my mikrotik router. Quite simple setup and very convenient.
