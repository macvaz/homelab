...
With a running docker environment up&running as described in <<docker_server, docker server section>>, it's time to describe all customer facing applications deployed in the homelab and how they cooperate in a logical architecture.

=== Containers

All application exposed to users are deployed as Docker containers. Most of them are open-source community driven applications that ship its own docker images in hubs like Docker Hub or Linux Servers. There is also one internal development (portfolio manager) that it's also deployed as a docker container.
...
All above containers are easily 
