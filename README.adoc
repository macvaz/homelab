...
Proxmox graphical installer comes with very good defaults, making disk management very easy. Proxmox is installed only in one disk (the data disks are configured post installation using the web interface). The final layout looks professional and at the same time, simple to understand:
