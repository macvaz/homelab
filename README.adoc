...
All physical networking is configured on the Mikrotik Hap AX2 device, **managing 3 different local IP networks**. It also negociates with the ISP via PPPoE  in order to get a public IP for the WAN interface. The most meaningful details of the physical networks are the following:
