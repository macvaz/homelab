...
Now is time to choose which file-based remote storage technology to expose to the clients. OMV mainly offers NFS and SMB. *SMB protocol was finally choosen due to its security* features. 

Finally, using OMV administration tool again, 3 drives were created for external access. Each drive was secured with a user and password. The main features of the SMB drives are listed below:  
