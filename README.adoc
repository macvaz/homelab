...
In a production-grade hardware architecture, several specialized appliances are normally deployed. However, a homelab is in a residential environment where space restrictions can be hard overcome compared with a professional datacenter. 

Selecting devices with small form factors is a must to reduce physical space requirements. In addition, power efficiency is also mandatory for this kind of devices as they operate 24x7 but normally running low computational loads. **Virtualization technologies may help in both aspects: space savings and power eficiency**.
...
Since both HCI native storage technologies supported by Proxmox where discarded, **bfinally oth data disks are not managed by the Proxmox hypervisor**. 
...
To get a detailed description of the storage design, check section "NAS server".
...
Virtualization node has only 1 NIC directly attached to my router. However, Proxmox allow to setup a https://pve.proxmox.com/wiki/Network_Configuration[bridged networking configuration], extending LAN network address space to the virtual servers started inside the hypervisor. **This networking setup creates a Linux bridge interface (vmbr0) to where all virtual servers are connected.** This bridge is also connected to the physical NIC, reusing DHCP server and internet gateway from my Mikrotik router.
