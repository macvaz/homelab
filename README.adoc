...
[source]
----
>> root@proxmox:~$ sudo smartctl -a /dev/sda
...
...
*I describe how to create a redundant storage pool* based on a pair of 1 TB external USB disks in section 2 (Storage pools).
...
Using Proxmox graphical interface makes network setup quite easy. It detected my router physical network and allowed to set up easily a fixed IP address for Proxmox. This IP is visible in all my LAN and is where Proxmox VE GUI can be found.
...
Since I relied on my physical router network (1921.68.1.0/24), I found no need to create virtual networks in the proxmox hypervisor. Only two physical address are used of my home network:

[source]
----
  - one for proxmox (192.168.1.4) => Hypervisor bare-metal deployment (hostname **proxmox**)
  - one for the virtual machine where docker runs (192.168.1.2) => Virtual machine created over Proxmox (hostname **minipc**)
----

**minipc** VM has one external IP and lots of private IP address (one for each docker container).
