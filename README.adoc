...
*There is only 1 virtualization node (proxmox1) where virtualization server (Proxmox VE) is installed*. Its hardware specs are the following:
...
Since Proxmox Virtual Environment (VE) is a https://en.wikipedia.org/wiki/Hypervisor[Type 1 hypervisor], it is installed bare-metal, *directly over hardware*. Installing *Proxmox Virtual Environment (PVE) 7.3* is not harder than any bare-metal Linux-based installation. *ventoy* was used to flash Proxmox PVE ISO file in a USB stick. proxmox1 was then booted from the USB drive and conducted a common installation using the KVM extender.
...
[source]
----
 - Reverse proxy managing SSL certificates (nginx proxy manager)
 - Docker web front-ends
 - Docker database backends (MariaDB and PostgreSQL)
----

All docker IPs are privave and not reacheble from docker virtual server without going troguth proxy container. Only pihole container is reachable from the rest of the homelab in order to expose DNS services to the client network components (mobile phones, tables and pcs)
