...
In the next logical architecture diagram, the *main software services and batch jobs running* in the homelab are presented. This diagram includes software running in both bare-metal infrastructure (hypervisor) and virtual infrastructure ("nas" and "docker" VMs). It also presents the main external services used by the system like NameCheap (Domain register), CloudFlare (DNS management), Let'sEncrypt (SSL certificates issuance) and Mega.io (off-site backup).
