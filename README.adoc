...
Since the minipc final location is in a very uncomfortable place (in the top of a bookshelf), I installed Proxmox VE similarly to how desktop machines are installed, reusing my usual keyboard, screen and wifi connection via DHCP.

=== OS Disk partitioning

Proxmox graphical installer comes with very good defaults, making disk management very easy. Proxmox is installed only in one disk (the data disks are configured post installation using the web interface as storage pools). The final layout looks professional and at the same time, simple to understand:
...
Each partition and volume group has a very common and clear goal:
[source]
----
...
- pve-data_tmeta (metadata) is a volume group based on thin provisioning used to store VMs virtual hard disks
- pve-data_tdata (data) is a volumen group based on thin provisioning used to store VMs VMs virtual hard disks
----
