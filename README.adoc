...
Proxmox graphical installer comes with very good defaults, making disk management very easy. Proxmox is installed only in one disk, letting the other disks to create storage pools using the web interface after installation. The final layout of the OS disk looks professional and at the same time, simple to understand:
...
Each partition or volume group has a very common and clear goal:
[source]
----
...
postfix sobre proxmox


root@proxmox:~# more /etc/postfix/sasl_passwd
