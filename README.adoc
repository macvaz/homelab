...
On the other way, Docker packages worked perfectly well inside LXC containers using the Debian 11 template. After running Linux Containers for a while, I can recommend this technology, very few limitations detected however isolation is lower than with virtual machines. Anyway, a complete and isolated docker server using only 80 MB of RAM, great resource utilization.
