...
NAS appliances are a common IT device that provides large size disk and fault-tolerance. Since you have to buy both the chassis and at least 2 disks, NAS appliances are expensive. If you don't mind to pay that cost, I recommend Synology NAS home appliances.

Extended the mimipc with 2 external 1 TB USB disks was only 80€ so I decided to transform minipc in a homemade NAS server. Created a NAS server in Linux is not hard and a very common option is to create a RAID device using mdadm and expose the drive through NFS and SMB.

Since /dev/sda is used only as main operating system disk drive, this new RAID device is used exclusively to support the NAS.
