...
|backups
|SMB
|Stores virtual servers backups for long term storage. As storage server is a VM, can't store directly into this drive when backing up this VM. Storage server backup is done in 2 steps.
|proxmox
