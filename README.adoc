...
Using Proxmox VE, creating a software-defined IT infrastructure where both storage and computing are virtualized on a unified hardware platform is possible. Consequently, no specialized hardware appliances are required for computation and storage.
...
|===
|Hypervisor name |IP addresses | OS | RAM | Physical disks
