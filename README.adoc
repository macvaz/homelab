...
Apart from virtual networking devices created by Proxmox VE, there also another networking virtualization technology (Docker) internal the docker VM (192.168.1.2/24). *This software-defined Docker network environment, contrary to proxmox1's is private* and never leaves docker VM, so any physical external physical device (mikrotik router, TV and mobile clients) are totally unaware of 10.10.10.0/24 network. This network is only used to docker containers to communicate each other.
