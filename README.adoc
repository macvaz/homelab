...
=== Virtualization node (proxmox1)

CThere is only one virtualization node in the deployment: *"proxmox1"*. Its hardware specs are the following:
...
*Low power consumption was a requirement for hardware selection*. CPU model and motherboard were specifically selected trying to maximize a relative metric (performance / power) rather than an absolute one (like clock speed or MIPS). *With a TDP (thermal design power) of 15 watts*, both CPU model (Intel Celeron J5040) and mother-board (Asrock J5040 ITX) were outstanding options.
