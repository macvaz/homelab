...
Due to the lack of free physical space at home, a software-defined IT infrastructure approach like Hyper-converged infrastructure (HCI) was ideal. HCI allows to virtualize all traditional hardware equipment using software-defined computing, networking and storage.

This approach doesn't required specialized hardware appliances for storage (ej: NAS appliance) that are very common in IT deployments. Both computing and storage infrastructure can be virtualized by software using a hypervisor. Since Proxmox VE is the most common open-source Type 1 hypervisor available, it is the base technology of the homelab.
