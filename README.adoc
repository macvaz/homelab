...
Now is time to choose which file-based remote storage technology to expose to the clients. OMV mainly offers NFS and SMB. *SMB protocol was finally choosen due to its security* features. Finally, using OMV administratiion toool, 3 SMB drives were created over the storage server file system: 
